...
def aPTT_Section():
    ...
    if report["LAPTT_R"] > report["LAPTT_U"]:
        aPTT_Report += (f"The initial clotting time in the aPTT-based testing system is prolonged at {report["LAPTT_R"]:.1f} seconds, which is {(report["LAPTT_R"] - report["LAPTT_U"]):.1f} seconds above the upper limit.")
        ...
        if report["LTT_R"] != -1:           
            if report["LTT_R"] < report["LTT_L"]:
                ...
                aPTT_Report += (f" The thrombin time is {">" if report["LTT_R"] == 100.0 else ""}{(report["LTT_R"] - report["LTT_U"]):.1f} seconds prolonged above the upper limit normal of {report["LTT_U"]:.1f} seconds.")
            else:
...
def DRVVT_DPT_Sections(testname, initR, initU, mixR, mixU, corrR, percentR, percentU):
    ...
    if initR > initU:
        ...
        battery_Report += (f"The initial clotting time is normal in the {testname}-based testing phase.")
        testStatus[f"{testname}"] = "Negative"
...
def Antigenic_Section(testR, testU, testName, testType, unit):
    ...
    if testR > testU:
        antigenicReport += (f"Solid phase testing for {testName} {testType} is elevated at {testR:.1f} {unit} (99th percentile upper limit of normal {testU} {unit}).")
...
def Conclusion_L1():
    ...
    if len(Positive_Tests) > 0:
        conclusion += (f" Confirmatory repeat LA/APS testing after at least 12 weeks is recommended{suffix if suffix != -1 else ""}.")
