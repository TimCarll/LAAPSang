...
def sanitize_date(date):
    ...
    if isinstance(date, float):
        ...
        match = re.search(r'\d{1,2}/\d{1,2}/\d{2,4}', date)
        if match:
...
def retest_Date(date):
    inputDate = datetime.datetime.strptime(date, "%m/%d/%y")
    newDate = inputDate + datetime.timedelta(weeks=12, days=1)
...
def readFile(filename):
    ...
    try: 
        ...
        a["DATE"] = sanitize_date(sheet.cell_value(5,5))
        a["SPEC_ID"] = sheet.cell_value(6,5)
        
        if a["DATE"] != -1:
            a["RETEST_DATE"] = retest_Date(a["DATE"])
        else:
            a["RETEST_DATE"] = -1
        ...
        a["LTTHEP_L"] = sanitize(sheet.cell_value(13,2))
...
#Writes a doc
def writeFile(path, content, italicContent, filename):

    interpretedDoc = Document()
    paragraph = interpretedDoc.add_paragraph()
    paragraph.add_run(content)

    if italicContent != -1:
        italicContent = paragraph.add_run(italicContent)
        italicContent.italic = True
